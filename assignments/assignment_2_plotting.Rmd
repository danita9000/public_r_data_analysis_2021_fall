...
ggplot(phds, aes(x = year, y = n_phds, group = broad_field)) +
  ...
  ggtitle("Phds awarded in the US(by year)") +
...
commute_data <- tt_load("2019-11-05")
commute <- commute_data$commute
commute <- commute %>%
  drop_na() %>%
...
ggplot(commute, aes(x = Walk, y = Bike, label = state_abb,color = state_region)) + 
  ...
  ggtitle("Nbr. people biking vs. walking to work by USA state") +
  ylab("Nbr. people biking to work  (log N)") + 
  xlab("Nbr. people walking to work (log N)") +
